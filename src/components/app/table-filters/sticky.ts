 ...
 * tương đương.
 *
 * TẮT ghim ở màn SP (`max-sm:static`): màn ~360px chỉ còn ~324px cho bảng, mà
 * cụm ghim trái + phải chiếm tới ~246px — nội dung chỉ còn khe ~78px để trượt,
 * kéo ngang là chui thẳng xuống dưới cột ghim, không đọc được gì. Ở SP thà cho
 * cuộn hết cả bảng còn hơn ghim.
 */
...
const SHADOW_LEFT = 'sm:shadow-[-8px_0_8px_-8px_rgba(0,0,0,0.15)]';
/** Bóng mép phải của cột ghim trái. */
export const SHADOW_RIGHT = 'sm:shadow-[8px_0_8px_-8px_rgba(0,0,0,0.15)]';
...
export const STICKY_ACTION_HEAD = `max-sm:static sticky right-0 z-10 max-sm:pr-0! ${HEAD_BG} ${SHADOW_LEFT}`;
export const STICKY_ACTION_CELL = `max-sm:static sticky right-0 z-10 bg-card max-sm:pr-0! transition-colors ${ROW_SYNC} ${SHADOW_LEFT}`;

/** Cột ghim trái ở vị trí thứ 1/2/3 (offset cộng dồn theo bề rộng cố định). */
export const STICKY_L0_HEAD = `max-sm:static sticky left-0 z-10 max-sm:pl-0! ${HEAD_BG}`;
export const STICKY_L0_CELL = `max-sm:static sticky left-0 z-10 bg-card max-sm:pl-0! transition-colors ${ROW_SYNC}`;
export const STICKY_L10_HEAD = `max-sm:static sticky left-10 z-10 ${HEAD_BG}`;
export const STICKY_L10_CELL = `max-sm:static sticky left-10 z-10 bg-card transition-colors ${ROW_SYNC}`;
/** Cột cuối của cụm ghim trái — mang bóng phân cách. */
export const STICKY_L24_HEAD = `max-sm:static sticky left-24 z-10 ${HEAD_BG} ${SHADOW_RIGHT}`;
export const STICKY_L24_CELL = `max-sm:static sticky left-24 z-10 bg-card transition-colors ${ROW_SYNC} ${SHADOW_RIGHT}`;
