...
function CoursesGridSection({
  ...
  const { data: rows, meta, errors } = use(promise);
    ...
    <>
          ...
          >
            <Card className="hover:border-primary/40 h-full cursor-pointer gap-0 overflow-hidden p-0 transition hover:shadow-md">
              ...
              <CardContent className="space-y-3 pb-5">
                ...
                <div className="flex items-center justify-between gap-2">
                  <span className="text-primary text-sm font-semibold">{formatVnd(row.price)}</span>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center">
          <DataPagination page={meta.page} totalPages={totalPages} onPageChange={onPageChange} />
        </div>
      )}
    </>
  );
...
export default function StudentCoursesClient({ urlState, coursesPromise }: Props) {
